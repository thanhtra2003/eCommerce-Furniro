...
populationBtn.addEventListener('click', function () {
  ...
  const top10Countries = sortCountries.slice(0, 10)
  const total = top10Countries.reduce((a, b) => a + b.population, 0)
  const worldPopulation = countries.reduce((total, country) => total + country.population, 0)

  const world = { name: 'World', population: worldPopulation }
  top10Countries.unshift(world)
  ...
  top10Countries.forEach(country => {
    ...
    bar.className = 'bar'
    const width = (country.population / total) * 100
  ...
  //   chartHTML += `
  //     <div class="chart-bar">
  //       <div class="label" style="width: 200px;">${country.name}</div>
...
languageBtn.addEventListener('click', function () {
  ...
  const totalCount = top10Languages.reduce((total, language) => total + language.count, 0)
  chartContainer.innerHTML = ''
  top10Languages.forEach(language => {
    ...
    const bar = document.createElement('div')
    bar.className = 'bar'
    const width = (language.count / totalCount) * 100
    bar.style.width = `${width}%`
    const value = document.createElement('div')
    value.textContent = language.count.toLocaleString()
    chartBar.appendChild(label)
    chartBar.appendChild(bar)
    chartBar.appendChild(value)
    chartContainer.appendChild(chartBar)
